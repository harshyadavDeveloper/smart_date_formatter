...
class AnalyticsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        // ── Streak Calculator ──────────────────────────────
        const DemoSection('StreakCalculator — Habit Tracking'),
        ...
        // ── Date Grouper ───────────────────────────────────
        const DemoSection('DateGrouper — Group by Time Period'),
        const SizedBox(height: 8),
        const DemoCodeBox(
          "DateGrouper.byDay(dates)           // {'2024-06-01': [...]}\n"
        ...
        // ── Holiday Helper ─────────────────────────────────
        const DemoSection('HolidayHelper — Holiday Awareness'),
        const SizedBox(height: 8),
        const DemoCodeBox(
          'date.isHoliday(holidays: list)     // true\n'
          'date.isWorkingDay(holidays: list)  // false\n'
          'HolidayHelper.addWorkingDays(date, 5, holidays: list)\n'
          'HolidayHelper.indianHolidays(2024) // 8 holidays',
        ),
        const SizedBox(height: 12),
        _holidayCard(),
        const SizedBox(height: 20),
        ...
        // ── Recurrence Helper ──────────────────────────────
        const DemoSection('RecurrenceHelper — Recurring Dates'),
        const SizedBox(height: 8),
        const DemoCodeBox(
          'RecurrenceHelper.daily(start: date, count: 5)\n'
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              _statBox('🔥', '$current', 'Current', Colors.orange),
              const SizedBox(width: 8),
              _statBox('🏆', '$longest', 'Longest', Colors.indigo),
              const SizedBox(width: 8),
              _statBox('✅', '$total', 'Total', Colors.green),
              const SizedBox(width: 8),
              _statBox(todayDone ? '⭐' : '❌', todayDone ? 'Yes' : 'No', 'Today',
                  todayDone ? Colors.green : Colors.red),
            ]),
            const Divider(height: 16),
            _row('Last completed', last?.format('EEE, dd MMM yyyy') ?? '-'),
            _row('Total streak runs', '${allStreaks.length}'),
            _row('Completion rate (Jun 1–15)',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...byMonth.entries.map((e) => _barRow(e.key, e.value,
                byMonth.values.reduce((a, b) => a > b ? a : b), Colors.indigo)),
            const Divider(height: 16),
            const Text('By Weekday:',
                style:
                    TextStyle(fontWeight: FontWeight.bold, color: Colors.teal)),
            const SizedBox(height: 6),
            ...byWeekday.entries.map((e) => _barRow(
                e.key.substring(0, 3),
                e.value,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _row('Indian holidays 2024',
                '${HolidayHelper.indianHolidays(2024).length} days'),
            _row('Today is holiday?',
  ...
  Widget _recurrenceCard() {
    ...
    final monthly =
        RecurrenceHelper.monthly(start: DateTime(2024, 1, 15), count: 4);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _recSection('Daily (5)', daily, Colors.indigo),
            const SizedBox(height: 10),
            _recSection('Weekly (4)', weekly, Colors.teal),
            const SizedBox(height: 10),
            _recSection('Monthly (4)', monthly, Colors.orange),
            const SizedBox(height: 10),
            _recSection('Yearly (3)', yearly, Colors.green),
          ],
        ),
      ),
    );
  ...
  Widget _recSection(String title, List<DateTime> dates, Color color) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title,
              style: TextStyle(
          ...
          Wrap(
            ...
            children: dates
                .map((d) => Container(
                      ...
                      child: Text(d.format('EEE dd MMM'),
                          style: TextStyle(
                              fontSize: 11,
                              color: color,
